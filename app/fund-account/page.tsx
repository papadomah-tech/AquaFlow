...
export default function FundAccountPage() {
  ...
  const [payments, setPayments] = useState<any[]>([])   // rider payments to factory
  ...
  const load = useCallback(async () => {
    ...
    // Fund balance = all cash in (deposits + rider payments) − expenses
    ...
    setExpenses(exps ?? [])
    setData({
      totalDeposited, totalRiderPayments, totalExpenses,
      totalCashIn, fundBalance,
      totalRetailRev, totalRetailColl,
    ...
    <AppLayout>
      <div className="page-header">
        <div>
          ...
          <span className="badge badge-blue mt-1">Admin Only</span>
        </div>
        <div className="flex gap-2">
          <button onClick={() => { setFormType('deposit'); setShowForm(true) }}
            className="btn btn-primary">+ Bank Deposit</button>
          <button onClick={() => { setFormType('rider-payment'); setShowForm(true) }}
            className="btn btn-secondary">+ Rider Payment</button>
      ...
      {loading ? <div className="text-center py-12 text-gray-400">Loading...</div> : data && (
        <>
          ...
          <div className={'rounded-2xl p-5 mb-5 text-white shadow-lg '
            + (data.fundBalance >= 0 ? 'bg-[#1F4E79]' : 'bg-red-700')}>
          ...
          <div className="card mb-4">
            ...
            <div className="overflow-x-auto">
              <table className="data-table">
                ...
                <tbody>
                  {deposits.length === 0
                    ? <tr><td colSpan={5} className="text-center py-6 text-gray-400">No deposits in this period</td></tr>
                    : deposits.map((d:any) => (
                    <tr key={d.id}>
          ...
          <div className="card mb-4">
            ...
            <div className="overflow-x-auto">
              <table className="data-table">
                ...
                <thead>
                  <tr>
                    <th>Date</th><th>Rider / Rep</th>
                    <th>Reference</th><th className="right">Amount</th>
                  </tr>
                </thead>
                <tbody>
                  {payments.length === 0
                    ? <tr><td colSpan={4} className="text-center py-6 text-gray-400">No rider payments in this period</td></tr>
                    : payments.map((p:any) => (
                    <tr key={p.id}>
          ...
          <div className="card">
            ...
            <div className="overflow-x-auto">
              <table className="data-table">
                ...
                <thead>
                  <tr><th>Date</th><th>Category</th><th>Description</th><th className="right">Amount</th></tr>
                </thead>
                <tbody>
                  {expenses.length === 0
                    ? <tr><td colSpan={4} className="text-center py-6 text-gray-400">No expenses in this period</td></tr>
                    : expenses.map((e:any) => (
                    <tr key={e.id}>
          ...
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="font-bold text-[#1F4E79]">
                {formType === 'deposit' ? '🏦 Record Bank Deposit' : '🛵 Record Rider Payment'}
              </h2>
