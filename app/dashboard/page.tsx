...
export default function DashboardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const { data: todayProd } = await supabase
      .from('production_batches').select('bags_produced').eq('batch_date', today())
    ...
    const { data: recSales } = await supabase
      .from('sales')
      .select('id,sale_date,customers(name),bags_sold,total_amount,payment_status')
  ...
  // Block non-admins
  if (!isAdmin) return (
    <AccessDenied message="The Dashboard is only accessible to Administrators." />
  )
